...
func (s *Store) List(limit int) ([]Event, error) {
	...
	// Keep a rolling window of the last `limit` lines without loading the whole file into memory twice.
	buf := make([]string, 0, limit)
	sc := bufio.NewScanner(f)
	// Events are small; allow slightly larger lines than default.
