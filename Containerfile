...
COPY --from=web /web/dist/ ./cmd/mini-acm/static/

ARG VERSION=dev
ARG BUILD_VERSION
RUN VER="${BUILD_VERSION:-${VERSION}}"; \
    CGO_ENABLED=0 go build -ldflags "-X main.version=${VER}" -o /build/mini-acm ./cmd/mini-acm
...
RUN microdnf install -y tar gzip ca-certificates && \
    ...
    rm -f /tmp/oc.tar.gz && \
    microdnf clean all && \
    useradd -u 65532 -r -s /sbin/nologin miniacm && \
